...
CREATE TABLE IF NOT EXISTS `crm_customer_owner_history` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '编号',
    `customer_id` BIGINT NOT NULL COMMENT '客户编号',
    `change_type` VARCHAR(32) NOT NULL COMMENT '变更类型：RECEIVE-领取，ASSIGN-分配，TRANSFER-转移，MANUAL_PUT-手动移入公海，AUTO_PUT-自动移入公海',
    `old_owner_user_id` BIGINT NULL COMMENT '变更前负责人用户编号',
    `new_owner_user_id` BIGINT NULL COMMENT '变更后负责人用户编号',
    `reason` VARCHAR(500) NULL COMMENT '原因',
    `operator_user_id` BIGINT NOT NULL COMMENT '操作人用户编号，0表示系统',
    ...
    INDEX `idx_tenant_new_owner_time` (`tenant_id`, `new_owner_user_id`, `change_time`)
